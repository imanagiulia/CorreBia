...
WIDTH = 900
HEIGHT = 950
# Inicializa a tela com a flag RESIZABLE para permitir redimensionamento
...
class Inimigo:
    ...
    def draw(self):
        # Redimensiona a imagem do inimigo para se ajustar à tela
        current_width, current_height = screen.get_size()
        scale_w = current_width / original_width
        scale_h = current_height / original_height
        scaled_img = pygame.transform.scale(self.img, (int(45 * scale_w), int(45 * scale_h)))
        scaled_turbo_img = pygame.transform.scale(turbo_img, (int(45 * scale_w), int(45 * scale_h)))
        scaled_check_img = pygame.transform.scale(check_img, (int(45 * scale_w), int(45 * scale_h)))

        if (not powerup and not self.dead) or (eaten_inimigo[self.id] and powerup and not self.dead):
            screen.blit(scaled_img, (self.x_pos * scale_w, self.y_pos * scale_h))
        elif powerup and not self.dead and not eaten_inimigo[self.id]:
            screen.blit(scaled_turbo_img, (self.x_pos * scale_w, self.y_pos * scale_h))
        else:
            screen.blit(scaled_check_img, (self.x_pos * scale_w, self.y_pos * scale_h))
        self.rect = pygame.rect.Rect(self.x_pos * scale_w, self.y_pos * scale_h, int(45 * scale_w), int(45 * scale_h))
        return self.rect

    def check_collisions(self):
        current_width, current_height = screen.get_size()
        # Use as dimensões originais para o cálculo das células para indexar 'level'
        cell_h_original = (original_height - 50) // 32
        cell_w_original = original_width // 30

        # Desescalar a posição do inimigo para as coordenadas originais do tabuleiro
        scale_w = current_width / original_width
        scale_h = current_height / original_height
        unscaled_x_pos = self.x_pos / scale_w
        unscaled_y_pos = self.y_pos / scale_h

        linha = max(0, min(int((unscaled_y_pos + 22) // cell_h_original), len(level) - 1))
        coluna = max(0, min(int((unscaled_x_pos + 22) // cell_w_original), len(level[0]) - 1))

        # Ajusta as coordenadas da caixa para o redimensionamento
        box_xmin = 350 * (current_width / original_width)
        box_xmax = 550 * (current_width / original_width)
        box_ymin = 370 * (current_height / original_height)
        box_ymax = 480 * (current_height / original_height)

        is_in_box = (box_xmin <= self.x_pos * (current_width / original_width) < box_xmax and box_ymin < self.y_pos * (
                    current_height / original_height) < box_ymax)
    ...
    def check_turns(self):
        # Use as dimensões originais para o cálculo das células para indexar 'level'
        cell_height_original = (original_height - 50) // 32
        cell_width_original = original_width // 30
        margem = 15  # Esta margem está nas unidades de pixel originais

        # Desescalar as coordenadas centrais do inimigo para corresponder à grade original do tabuleiro
        current_width, current_height = screen.get_size()
        scale_w = current_width / original_width
        scale_h = current_height / original_height
        unscaled_center_x = self.center_x / scale_w
        unscaled_center_y = self.center_y / scale_h

        linha = int(unscaled_center_y // cell_height_original)
        coluna = int(unscaled_center_x // cell_width_original)
        ...
        # Estas verificações também precisam usar coordenadas desescaladas e dimensões de célula originais
        if 10 <= unscaled_center_x % cell_width_original <= 16:
            if level[int((unscaled_center_y + margem) // cell_height_original)][coluna] < 3:
                turns[3] = True
            if level[int((unscaled_center_y - margem) // cell_height_original)][coluna] < 3:
                turns[2] = True
        if 10 <= unscaled_center_y % cell_height_original <= 16:
            if level[linha][int((unscaled_center_x + margem) // cell_width_original)] < 3:
                turns[0] = True
            if level[linha][int((unscaled_center_x - margem) // cell_width_original)] < 3:
                turns[1] = True
    ...
    def move_kernel(self):
        ...
        if self.x_pos < -30:
            self.x_pos = current_width - 45
        elif self.x_pos > current_width - 15:
            self.x_pos = -25

    def move_cloudius(self):
        self.center_x = self.x_pos + 22
        self.center_y = self.y_pos + 22
...
def draw_alter():
    ...
    screen.blit(score_text, (30 * scale_w, 750 * scale_h))
    if powerup:
        pygame.draw.circle(screen, 'red', (int(140 * scale_w), int(720 * scale_h)), int(15 * min(scale_w, scale_h)))
    for l in range(lives):
        scaled_player_img = pygame.transform.scale(player_images[1], (int(40 * scale_w), int(40 * scale_h)))
        screen.blit(scaled_player_img, (int((650 + l * 40) * scale_w), int(750 * scale_h)))
    if game_over:
        pygame.draw.rect(screen, 'dark gray',
                         [int(50 * scale_w), int(300 * scale_h), int(800 * scale_w), int(300 * scale_h)], 0,
                         int(10 * min(scale_w, scale_h)))
        pygame.draw.rect(screen, 'orange',
                         [int(70 * scale_w), int(320 * scale_h), int(760 * scale_w), int(260 * scale_h)], 0,
                         int(10 * min(scale_w, scale_h)))
        gameover_text1 = font2.render('Game Over!', True, 'red')
        gameover_text2 = font.render(' Aperte [espaço] para recomeçar!', True, 'red')
        gameover_rect1 = gameover_text1.get_rect(center=(current_width // 2, current_height // 2 - 50 * scale_h))
        gameover_rect2 = gameover_text2.get_rect(center=(current_width // 2, current_height // 2 + 50 * scale_h))
    ...
    if game_won:
        ...
        gamewon_rect1 = gamewon_text1.get_rect(center=(current_width // 2, current_height // 2 - 50 * scale_h))
        gamewon_rect2 = gamewon_text2.get_rect(center=(current_width // 2, current_height // 2 + 50 * scale_h))
...
def check_collisions(scor, power, power_count, eaten_inimigo):
    ...
    # Use as dimensões originais para o cálculo das células para indexar 'level'
    cell_height_original = (original_height - 50) // 32
    cell_widht_original = original_width // 30

    # Desescalar as coordenadas centrais do jogador para corresponder à grade original do tabuleiro
    scale_w = current_width / original_width
    scale_h = current_height / original_height
    unscaled_center_x = center_x / scale_w
    unscaled_center_y = center_y / scale_h

    if 0 < unscaled_center_x < original_width - 30:  # Ajuste para a largura original
        if level[int(unscaled_center_y // cell_height_original)][int(unscaled_center_x // cell_widht_original)] == 1:
            level[int(unscaled_center_y // cell_height_original)][int(unscaled_center_x // cell_widht_original)] = 0
            scor += 10
        if level[int(unscaled_center_y // cell_height_original)][int(unscaled_center_x // cell_widht_original)] == 2:
            level[int(unscaled_center_y // cell_height_original)][int(unscaled_center_x // cell_widht_original)] = 0
            scor += 50
            power = True
            power_count = 0
...
def draw_board(level):
    current_width, current_height = screen.get_size()
    num1 = ((current_height - 50) // 32)
    num2 = (current_width // 30)
        ...
        for j in range(len(level[i])):
            if level[i][j] == 1:
                pygame.draw.circle(
                    ...
                    radius=int(4 * min(current_width / original_width, current_height / original_height))
                )
            if level[i][j] == 2 and not flicker:
                pygame.draw.circle(
                    ...
                    color='yellow',
                    center=(j * num2 + (0.5 * num2), i * num1 + (0.5 * num1)),
                    radius=int(10 * min(current_width / original_width, current_height / original_height))
                )
            if level[i][j] == 3:
                pygame.draw.line(
                    surface=screen,
                    ...
                    width=int(3 * min(current_width / original_width, current_height / original_height))
                )
            if level[i][j] == 4:
                pygame.draw.line(
                    surface=screen,
...
def draw_player():
    ...
    scale_w = current_width / original_width
    scale_h = current_height / original_height
    scaled_player_img = pygame.transform.scale(player_images[counter // 15 % len(player_images)],
                                               (int(45 * scale_w), int(45 * scale_h)))

    if direction == 0:
        screen.blit(scaled_player_img, (player_x * scale_w, player_y * scale_h))
    elif direction == 1:
        screen.blit(pygame.transform.flip(scaled_player_img, True, False), (player_x * scale_w, player_y * scale_h))
    elif direction == 2:
        screen.blit(pygame.transform.rotate(scaled_player_img, 90), (player_x * scale_w, player_y * scale_h))
    elif direction == 3:
        screen.blit(pygame.transform.rotate(scaled_player_img, -90), (player_x * scale_w, player_y * scale_h))


def check_position(centerx_scaled, centery_scaled):
    turns = [False, False, False, False]

    # Obter as dimensões atuais da tela e calcular os fatores de escala
    current_width, current_height = screen.get_size()
    scale_w = current_width / original_width
    scale_h = current_height / original_height

    # Desescalar as coordenadas centrais para corresponder à grade original do tabuleiro
    unscaled_centerx = centerx_scaled / scale_w
    unscaled_centery = centery_scaled / scale_h

    # Calcular as dimensões da célula com base no tamanho original do tabuleiro para indexar 'level'
    cell_height_original = (original_height - 50) // 32
    cell_width_original = original_width // 30

    margem = 15  # Esta margem está nas unidades de pixel originais

    # Calcular os índices de linha e coluna para a matriz 'level' usando coordenadas desescaladas
    linha = int(unscaled_centery // cell_height_original)
    coluna = int(unscaled_centerx // cell_width_original)

    # Garantir que os índices calculados estejam dentro dos limites da matriz 'level'
    linha = max(0, min(linha, len(level) - 1))
    coluna = max(0, min(coluna, len(level[0]) - 1))

    # Todas as verificações contra 'level' devem usar índices derivados de coordenadas desescaladas
    # e dimensões de célula originais.
    if 0 <= unscaled_centerx < original_width and 0 <= unscaled_centery < original_height - 50:
        # Verificar direita
        if level[linha][int((unscaled_centerx + margem) // cell_width_original)] < 3:
            turns[0] = True
        # Verificar esquerda
        if level[linha][int((unscaled_centerx - margem) // cell_width_original)] < 3:
            turns[1] = True
        # Verificar cima
        if level[int((unscaled_centery - margem) // cell_height_original)][coluna] < 3:
            turns[2] = True
        # Verificar baixo
        if level[int((unscaled_centery + margem) // cell_height_original)][coluna] < 3:
            turns[3] = True

        # Verificações adicionais para estar perto dos centros das células
        if 10 <= unscaled_centerx % cell_width_original <= 16:
            if level[int((unscaled_centery + margem) // cell_height_original)][coluna] < 3:
                turns[3] = True
            if level[int((unscaled_centery - margem) // cell_height_original)][coluna] < 3:
                turns[2] = True
        if 10 <= unscaled_centery % cell_height_original <= 16:
            if level[linha][int((unscaled_centerx + margem) // cell_width_original)] < 3:
                turns[0] = True
            if level[linha][int((unscaled_centerx - margem) // cell_width_original)] < 3:
                turns[1] = True
...
def move_player(play_x, play_y):
    ...
    play_x = max(-50, min(play_x, original_width + 50))
    play_y = max(0, min(play_y, original_height - 50))
...
def get_targets(player_x, player_y):
    ...
    runaway_x = original_width if player_x < original_width / 2 else 0
    runaway_y = original_height if player_y < original_height / 2 else 0
...
while run:
    ...
    # Ajusta center_x e center_y com base nas proporções da tela
    current_width, current_height = screen.get_size()
    scale_w = current_width / original_width
    scale_h = current_height / original_height
    center_x = int((player_x + 25) * scale_w)
    center_y = int((player_y + 25) * scale_h)
    ...
    player_circle = pygame.draw.circle(screen, 'black', (center_x, center_y), int(21 * min(scale_w, scale_h)), 2)
    draw_player()
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            run = False
        # Adiciona o manipulador de eventos para redimensionamento da janela
        if event.type == pygame.VIDEORESIZE:
            WIDTH, HEIGHT = event.size
            screen = pygame.display.set_mode((WIDTH, HEIGHT), pygame.RESIZABLE)
            # Reajusta as posições do jogador e inimigos para a nova escala
            player_x = int(player_x * (WIDTH / original_width))
            player_y = int(player_y * (HEIGHT / original_height))
            cloudius.x_pos = int(cloudius.x_pos * (WIDTH / original_width))
            cloudius.y_pos = int(cloudius.y_pos * (HEIGHT / original_height))
            ping.x_pos = int(ping.x_pos * (WIDTH / original_width))
            ping.y_pos = int(ping.y_pos * (HEIGHT / original_height))
            glitch.x_pos = int(glitch.x_pos * (WIDTH / original_width))
            glitch.y_pos = int(glitch.y_pos * (HEIGHT / original_height))
            kernel.x_pos = int(kernel.x_pos * (WIDTH / original_width))
            kernel.y_pos = int(kernel.y_pos * (HEIGHT / original_height))
            # Atualiza as dimensões originais para o próximo redimensionamento
            original_width = WIDTH
            original_height = HEIGHT
    ...
    current_width, _ = screen.get_size()
    if player_x > current_width:
        player_x = 45
    elif player_x < 0:
        player_x = current_width - 50
